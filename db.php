...
class admin {
    ...
    function dtypes(){
        $db = new PDO("mysql:host=localhost;dbname=pfe;charset=utf8",'root','');
        $response = $db->prepare('select * from dtype WHERE doc_type="download";');
        $response->execute();
        return $response;
    }
    function documentsList(){
        $db = new PDO("mysql:host=localhost;dbname=pfe;charset=utf8",'root','');
        $response = $db->prepare('SELECT * from document INNER JOIN dtype ON dtype.Id_type=document.id_type AND dtype.doc_type="download"');
        $response->execute();
        return $response;
    }
}

...
class admin {
    ...
    function getStudents($classe){
        $db = new PDO("mysql:host=localhost;dbname=pfe;charset=utf8",'root','');
        $response = $db->prepare('select * from etudiant where Id_class=?');
        $response->execute([$classe]);
        return $response;
    }
    ...
    function Mat($matiere){
         $db = new PDO("mysql:host=localhost;dbname=pfe;charset=utf8",'root','');
        $response = $db->prepare('select * from matiere where Code_Mat=?');
        $response->execute([$matiere]);
        return $response;

    }
  
    function modifiernote($Id_eval){
     $db = new PDO("mysql:host=localhost;dbname=pfe;charset=utf8",'root','');
     $response = $db->prepare('select * from evaluation inner join etudiant on evaluation.CNE = etudiant.CNE  where Id_eval=? ');
        $response->execute([$Id_eval]);
       return $response;
}
